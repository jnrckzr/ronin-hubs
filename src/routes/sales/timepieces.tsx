...
function StockBadge({ state }: { state: StockState }) {
  ...
  // state is now narrowed to ok | exact | over — all have `available`
  return (
...
function StockBanner({ state }: { state: StockState }) {
  ...
  // exact — state.available is safe here
  return (
...
function Page() {
  ...
  // ── Inventory map: "brand||model" → available qty ──
  ...
  const effectiveQty  = isOverStock
    ? (stockState as { kind: "over"; available: number }).available
    : Number(qty) || 1;
  ...
  const submit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (isOutOfStock)
      return toast.error("Out of stock — restock before recording a sale");
    ...
    <div className="tp-root">
      <style>{`
        ...
        .tp-root .stat-value {
          ...
          letter-spacing: -0.02em; line-height: 1.2; word-break: break-all;
        }
        ...
        .tp-root .panel-heading {
          font-size: 0.82rem; font-weight: 700; letter-spacing: .1em;
          text-transform: uppercase; color: var(--c-text3);
          margin: 0 0 18px;
        ...
        @media (min-width: 1020px) {
          .tp-root .form-grid {
            grid-template-columns: 118px 135px 1fr 68px 148px 148px 1fr 108px;
          }
        }
        ...
        .tp-root .field-label {
          ...
          display: block; margin-bottom: 6px; transition: color .18s;
        }
        ...
        /* ── Model field wrapper (for badge positioning) ── */
        .tp-root .model-field-wrap { position: relative; }
        .tp-root .model-field-wrap .stock-badge {
          position: absolute;
          top: -22px; right: 0;
        }
        ...
        /* ── Stock banner ── */
        .tp-root .stock-banner {
          display: flex; align-items: flex-start; gap: 8px;
          padding: 10px 14px;
          border-radius: var(--r-sm);
          font-size: 13px; font-weight: 600; line-height: 1.4;
          animation: tp-bannerIn .2s ease;
          margin-top: 2px;
          grid-column: 1 / -1;
        }
        ...
        /* ── Select override ── */
        .tp-root .form-field [data-radix-select-trigger] {
          height: 42px !important; background: var(--c-elevated) !important;
          border: 1.5px solid var(--c-border) !important;
          border-radius: var(--r-sm) !important;
          color: var(--c-text) !important; font-family: inherit !important;
        ...
        .tp-root .form-field [data-radix-select-trigger]:focus,
        .tp-root .form-field [data-radix-select-trigger][data-state='open'] {
          border-color: var(--c-accent) !important;
          box-shadow: 0 0 0 3px var(--c-accent-lt) !important;
        }
        ...
        .tp-root .submit-btn:hover:not(:disabled) {
        ...
        .tp-root .submit-btn.btn-capped {
          background: var(--c-warn);
        }
        .tp-root .submit-btn.btn-capped:hover:not(:disabled) {
          background: #b8863a;
          box-shadow: 0 4px 20px rgba(201,153,93,.35);
        }
        .tp-root .submit-btn.btn-disabled-stock {
          background: var(--c-danger); opacity: 0.6;
        }
        .tp-root .submit-btn::after {
          content: '';
        ...
        .tp-root .sales-table th {
          padding: 13px 16px; font-size: 0.67rem; font-weight: 800;
          letter-spacing: .12em; text-transform: uppercase;
          color: var(--c-text3); text-align: left; white-space: nowrap;
        }
        .tp-root .sales-table td {
        ...
        .tp-root .sales-table tbody tr {
          position: relative; transition: background .15s;
          animation: tp-rowIn .32s ease both;
        }
        .tp-root .sales-table tbody tr::before {
        ...
        .tp-root .buyer-badge {
          ...
          border: 1px solid rgba(110,142,247,0.22);
          transition: background .15s, border-color .15s, box-shadow .15s;
          cursor: default;
        }
        .tp-root .buyer-badge:hover {
        ...
        .tp-root .pay-chip {
          ...
          background: var(--c-elevated); color: var(--c-text2);
          border: 1px solid var(--c-border);
        }
        ...
        .tp-root .remove-btn {
          ...
          border-radius: var(--r-sm); color: var(--c-text3); cursor: pointer;
          transition: background .15s, color .15s, transform .12s, border-color .15s, box-shadow .15s;
        }
        .tp-root .remove-btn:hover {
          background: rgba(199,107,107,.12); color: var(--c-danger);
          border-color: var(--c-danger);
          box-shadow: 0 0 10px rgba(199,107,107,.18);
        ...
        .tp-root .skel {
          ...
          background: linear-gradient(90deg, var(--c-elevated) 25%, var(--c-border) 50%, var(--c-elevated) 75%);
          background-size: 200% 100%;
          animation: tp-skelMove 1.5s infinite;
        }
        ...
        @media (max-width: 767px) and (orientation: landscape) {
          .tp-root .page-title { font-size: 1.5rem; }
          .tp-root .stat-strip { grid-template-columns: repeat(2, 1fr); }
        }
        @media (max-width: 359px) {
          .tp-root .page-title { font-size: 1.35rem; }
          .tp-root .panel { padding: 14px; }
        }
      `}</style>
      ...
      <div className="panel tp-fade-up" style={{ animationDelay: "120ms" }}>
        ...
        <div className="form-grid">
          ...
          {/* Model — stock badge shown above input */}
          <div className="form-field">
          ...
          {/* Stock banner — spans full row */}
          {brand && model && (
            <StockBanner state={stockState} />
          )}
        </div>
      </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="sales-table" aria-label="Timepieces sales records">
            <thead>
              <tr>
                {[
                  "Date", "Brand", "Model", "Qty",
                  "Unit", "Total", "Buyer", "Payment", "",
                ].map((h, i) => (
                  <th key={i}>{h}</th>
                ))}
              </tr>
